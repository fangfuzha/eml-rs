...
- 交互式对照：通过 CLI 的 `--bytecode-parallel off|auto|force` 配合 `eval_parallel` / `eval_workers` 输出观察默认策略切换点。
...
- Interactive comparison: use the CLI `--bytecode-parallel off|auto|force` switch together with `eval_parallel` / `eval_workers` to inspect when the default strategy changes mode.
