...
- 交互式对照：通过 CLI 的 `--bytecode-parallel off|auto|force` 配合 `eval_parallel` / `eval_workers` 输出观察默认策略切换点。当前 Bytecode `auto` 的默认值为 `workers <= 8`、`min_samples_per_worker = 256`。
...
- Interactive comparison: use the CLI `--bytecode-parallel off|auto|force` switch together with `eval_parallel` / `eval_workers` to inspect when the default strategy changes mode. The current Bytecode `auto` defaults are `workers <= 8` and `min_samples_per_worker = 256`.
