...
- `off`: 禁用默认样本级并行。
- `auto`: 使用 Bytecode 专用默认阈值策略。当前默认值为“最多 `8` 个 workers，且每个 worker 至少分到 `256` 个样本”。
...
- `auto`: use the Bytecode-specific default threshold policy. The current default caps workers at `8` and requires at least `256` samples per worker.
