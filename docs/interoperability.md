...
```json
{
  ...
  "root": 3,
  "nodes": [{ "id": 0, "op": "var", "inputs": [], "attrs": { "index": 0 } }]
}
```
...
CLI 可直接生成 portable graph JSON，降低外部工具消费门槛：

```bash
cargo run --bin eml -- export portable "hypot(x0, x1)" > target/source-graph.json
cargo run --bin eml -- export portable "exp(x0) - log(x1)" --kind eml > target/eml-graph.json
```
...
The CLI can generate portable graph JSON directly for external tools:
