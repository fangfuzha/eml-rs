...
def collect_bytecode_policy_analysis(
  ...
  for entry in bytecode_modes:
    ...
    batch_recommendations.append({
      ...
      "confidence": recommendation_confidence(best["median"], second_best["median"]),
      "winner_margin_vs_second": ratio(best["median"], second_best["median"]),
    })
  ...
  default_reason = (
      "keep-auto-default" if keep_auto_default else "revisit-bytecode-auto-threshold")
  ...
  return {
    ...
    "default_policy_recommendation": {
      "recommended_mode": "auto" if keep_auto_default else batch_recommendations[-1]["recommended_mode"],
